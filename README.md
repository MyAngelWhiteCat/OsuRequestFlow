...
1) Внедрить Boost::json и Boost::log, для логгирования и сериализации данных (для фронта)
2) Рефакторинг последнего огромного коммита. Анализ багов, создать побольше тесткейсов.
3) Придумать как разбить класс Client на подклассы, пока он не разросся до безумия.
